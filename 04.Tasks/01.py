#!/usr/bin/env python
# Данный является логическим продолжением темы второго семинара задачи № 2,
# где мы парсили публикации с сайта https://gb.ru/posts с помощью BeautifulSoup.
# В данной работе я скрейпил уже страницы форума https://gb.ru/topics, 
# на них информация представлена в виде таблицы. Далее, произвожу парсинг и извлекая
...
from datetime import datetime # библиотека для работы с форматами датами, нужна для их преобразования
import locale # библиотека для работы с локализацией, необходима для правильного преобразования месяца
locale.setlocale(locale.LC_TIME, 'ru_RU.utf8') # указываю русскую локализацию, для преобразования месяцев

url = 'https://gb.ru' # адрес форума ГБ
file = '/topics' # адрес страницы форума
