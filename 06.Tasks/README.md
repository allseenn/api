...
Решение запросит ввод темы для поиска, после чего осуществит поиск по сайту www.unsplash.com фотографий и сохранит их результат в папке с именем темы, а также создаст CSV-файл с информацией о скачанных изображениях.
...
В решении не используется пагинация, т.к. на сайте реализована "бесконечная" прокрутка на базе JavaScript. JS Scrapy не поддерживает, но возможноа связка его с Selenium, который способен эмулировать работу пользователя за браузером.

Помимо загрузки фото, в корне решения создается табулированный CSV-файлы с информацией об изображениях:
...
- splash/items.py - файл определяющий основные сущности (поля) с которыми по отдельности можно производить манипуляции
- splash/spiders/middleware.py - в этом файле приписываются модули, методы, библиотеки и даже программы на других языках программирования
...
- splash/spiders/ - пустая папка с пауками
...
Созддим одного паука по имен unsplash для домена www.unsplash.com:
...
Чтобы передавать параметры в паука из вне, переопределим автоматически созданный класс UnsplashSpider:
...
В этом классе изменить метод parse, так чтоб он начал выковиривать ссылку по тем тегам и артибутам, которые их содержат:
...
Добавим метод parse_photo, который при переходе по ссылкам в методе parse будет на вновь открытых страницах изскать информацию которую мы хотим вытащить для дальнейшей обработки и сохранения:
...
После обработки блоков данных они передаются в piplenine.py для работы с этими данными, если это фото или видео, то в пайплайне они скачиваются, если это название и дата, то они в зависимости от настроек эти данные записываются в базу данных или выходной файл
...
Scrapy организован таким образом, чтобы при создании и настройке проекта, при последующией эксплуатации не править основной код, предусмотрен настраиваемые параметры и исзменяемые свойства, а так методы работы с ними через файл основной конфигурации settings.py

###### Основная конфигурация (settings.py)

Помимо настроек по-умолчиню, наш проект можно отрегулировать под собственные нужды.

Для точно парсинга рекомендую в настройках указать, тот браузер, с помощью которого исследуем целевой сайт, для правильного скрейпирования, иначе будут появляться другие теги, либо атрибуты, из-за целевого отображения сайта под конкретный браузер:

```
USER_AGENT = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
```

Количество одновременных подключений указываю 10, т.к. сайт proxyscrape.com на бесплатном тарифе выделяет 100 прокси, но до 10 одновременных к ним подключениям, больше сессий ставить нет смысла:
...
В результате тонких настроек CSV файл будет иметь разделитель полей табуляцию, не будет включать заголовки и будет задано то имя файла, которое передается вместе с переменной query.